...
import { useAuth, type Staff, type User } from '../Context/AuthContext';

interface AddInstructerProps {
  setOpenInstructerPopup: (open: boolean) => void;
  staffID: number | null;
  setStaffID: (id: number | null) => void;
}

const AddInstructer: React.FC<AddInstructerProps> = ({ setOpenInstructerPopup, staffID, setStaffID }) => {
  ...
  const handleAddInstructer = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError('');
    setMessage('');

    try {
      await axios.post<Staff>('/api/staff/add-courses', {
        staff_id: staffID,
