...
import { useAuth, type User, type Batch } from '../Context/AuthContext';

interface EnrollStudentProps {
    setOpenEnrollPopup: (open: boolean) => void;
    studentID: number | null;
    setStudentID: (id: number | null) => void;
}

const EnrollStudent: React.FC<EnrollStudentProps> = ({ setOpenEnrollPopup, studentID, setStudentID }) => {
    ...
    const { user } = useAuth();
    const student: User | undefined = students.find(s => s.user_id === user?.id);
    const batch: Batch | undefined = batches.find(b => b?.id === parseInt(selectedBatch));
    ...
    if(!user) return <div>Please log in to enroll students.</div>
    ...
    const handleEnroll = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setMessage('');
        try{
            await axios.post<Batch>('/api/students/enroll', {
